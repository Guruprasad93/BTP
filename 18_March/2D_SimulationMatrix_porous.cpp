...
void view_Matrix(double conc_grad[l_matrix][b_matrix])
{
    int i, j, k;
    ...
    for (j=0; j<b_matrix; j++)
    {
        for (k=0;k<l_matrix;k++)
        {
            ...
            cout<<"DIV_ctr:"<<divctr[k][j]<<"Position:"<<k<<" "<<j<<" Conc: "<<conc_grad[k][j]<<" Pores: "<<porous_matrix[k][j];
            cout<<endl;
        }
    }
...
int check_chemicalgradient(int* empty_x, int* empty_y, int empty_num_neighbours, double conc_grad[l_matrix][b_matrix])
{
    int i, max, index;
    max = -10;
    int pos_x, pos_y;
    for (i=0; i<empty_num_neighbours; i++)
    {
        pos_x = empty_x[i];
        pos_y = empty_y[i];
        if (conc_grad[pos_x][pos_y]>max)
        {
            max = conc_grad[pos_x][pos_y];
            index = i;
        }
    }
    return index;
...
void changedirofmigration(int k, int j, double conc_grad[l_matrix][b_matrix])
{
    ...
    else
    {
        ...
        randomposition = check_chemicalgradient(empty_x, empty_y, empty_num_neighbours, conc_grad); //directed Migration
...
void read_ConcMatrix(double conc_grad[l_matrix][b_matrix], char filename[100])
{
    string line;
    ifstream infile;
...
void start_automaton()
{
    ...
    for (int time =0; time<time_of_sim; time++)
    {
        ...
        sprintf(Conc_filename, "/home/guru/BTP/%d_March/Conc_Matrix/Conc_Matrix_%d.txt", 18, time);
        double conc_grad[l_matrix][b_matrix];
        ...
        for (j=0; j<b_matrix; j++)
        {
                ...
                else
                {
                    ...
                    if (op1 == 1)
                    {
                        ...
                        else
                        {
                            ...
                            if (op3 == 0)
                            {
                                changedirofmigration(k,j,conc_grad);
                            }
                            else
                            {
         ...
         //Writing to a file
        sprintf (filename, "/home/guru/BTP/18_March/2D_Plots/file%d.txt", time);
...
int main()
{
    ...
    FILE *fid;
    fid = fopen("/home/guru/BTP/18_March/2D_Plots/Initial.txt", "w");
